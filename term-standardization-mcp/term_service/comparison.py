...
def compare(new_term, new_definition, existing_term_id):
    ...
    fingerprint=hashlib.sha256(json.dumps([POLICY_VERSION,model,payload],ensure_ascii=False,sort_keys=True).encode()).hexdigest()
    ...
    try:
        ...
        result=ComparisonResult(**base,**judgment.model_dump(),recommended_action=action,method="structured_llm",model=model)
        with db.connect() as conn:
        ...
        return ComparisonResult(**base,relation="UNCERTAIN",confidence=0,reason="의미 비교 요청 실패로 자동 판단할 수 없음",
            differences=[],recommended_action="REVIEW_REQUIRED",method="unavailable",model=model,
