...
- **알려진 미해결(이번 변경과 무관, 확인됨)**: `create_term_request`(그리고 같은 패턴의 단어/도메인 신청 라우트)가 `INVALID_FIELDS` 응답에 `exc.errors()`를 그대로 실어 보내는데, pydantic이 `field_validator`의 `ValueError`를 `ctx`에 객체로 담아서 **동의어에 100자 초과 항목이 있으면 JSON 직렬화가 실패**(`Object of type ValueError is not JSON serializable`, 실측)해 500이 됩니다. 프론트는 동의어 길이를 미리 검사하지 않아서 이 경로가 열려 있음 — 고치려면 `admin_api.py`에서 `exc.errors(include_context=False)`(또는 `jsonable_encoder`)를 쓰고 프론트 `validateTermForm()`에 동의어 검사를 추가하세요.
