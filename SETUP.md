...
프론트엔드는 로그인해야만 들어갈 수 있고, 일반 가입은 항상 "승인 대기" 상태로 시작합니다(관리자가 승인해야 로그인 가능). 그런데 새로 만든 DB에는 그 승인을 해줄 관리자가 아직 한 명도 없으므로, 아래 명령으로 **최초 관리자 계정을 직접 만들어야** 로그인 화면을 통과할 수 있습니다. 이 단계를 건너뛰면 아무도 로그인할 수 없습니다.

**Mac/Linux**
```bash
cd term-standardization-mcp
.venv/bin/python manage.py create-admin "<아이디>" "<비밀번호>" "<이름>" "<팀(선택)>"
```

**Windows (PowerShell)**
```powershell
cd term-standardization-mcp
.venv\Scripts\python.exe manage.py create-admin "<아이디>" "<비밀번호>" "<이름>" "<팀(선택)>"
```

이후 다른 사람은 프론트엔드의 "회원가입" 탭으로 가입 신청하면 되고, 이 관리자 계정으로 로그인해 "회원 관리" 화면에서 승인/반려·정지/권한 변경을 처리합니다. 인증 아키텍처(비밀번호 해싱, 세션 쿠키, 역할) 자세한 내용은 [CLAUDE.md](CLAUDE.md)를 참고하세요.
